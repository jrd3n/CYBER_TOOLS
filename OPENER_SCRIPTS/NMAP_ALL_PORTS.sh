...
if [ -f "$file_name.xml" ]; then
    ...
    if [ "$user_input" == "y" ] || [ "$user_input" == "Y" ]; then
        ...
        sudo nmap $fetcher_ip -p 0-65535 -sS -sU -Pn --stats-every=5s -oA $file_name
    ...
    sudo nmap $fetcher_ip -p 0-65535 -sS -sU -Pn --stats-every=5s --max-parallelism 255 -T3 --min-rate=3000 -oA $file_name
